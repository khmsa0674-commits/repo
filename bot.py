...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        err = str(e)
        friendly = parse_ydl_error(err, platform)
        await status_msg.edit_text(friendly, parse_mode=ParseMode.MARKDOWN)
        logger.error(f"yt-dlp error | user={user.id} | platform={platform} | {err[:200]}")
...
def _find_downloaded_file(tmp_dir: str) -> str | None:
    """ابحث عن أي ملف فيديو تم تحميله في المجلد المؤقت."""
    ...
    if not candidates:
        return None
    # الأكبر حجماً هو الملف الصحيح عادةً
    return str(max(candidates, key=lambda f: f.stat().st_size))


def _convert_to_mp4(src_path: str, tmp_dir: str) -> str:
    """حوّل أي فيديو إلى mp4 مضغوط قابل للتشغيل مباشرة."""
    import subprocess
    out_path = os.path.join(tmp_dir, 'output_final.mp4')
    cmd = [
        'ffmpeg', '-y', '-i', src_path,
        '-c:v', 'libx264', '-preset', 'fast',
        '-crf', '28',
        '-c:a', 'aac', '-b:a', '128k',
        '-movflags', '+faststart',   # يخلي التشغيل فوري من تليكرام
        '-pix_fmt', 'yuv420p',       # متوافق مع كل المشغلات
        out_path
    ]
    result = subprocess.run(cmd, capture_output=True, timeout=180)
    if result.returncode == 0 and os.path.exists(out_path):
        return out_path
    return src_path  # إذا ffmpeg فشل ارجع الأصلي


def _sync_download(url: str, platform: str) -> dict:
    """تحميل الفيديو بإعدادات محسّنة لكل منصة."""
    import tempfile, subprocess
    tmp_dir = tempfile.mkdtemp()
    # استخدم اسم ثابت لتفادي مشاكل الترميز في أسماء الملفات
    ...
    if platform == 'tiktok':
        ydl_opts = {
            **base_opts,
            # نحمل النسخة بدون علامة مائية مباشرة
            'format': 'download_addr-0/play_addr-0/best',
            'extractor_args': {
                'tiktok': {
    ...
    # ─── إعدادات Instagram ─────────────────────────────────
        ...
        # كوكيز إذا موجودة تساعد على الريلز الخاصة
        for cookie_file in ['instagram_cookies.txt', 'cookies.txt']:
    ...
    # ─── إعدادات YouTube ───────────────────────────────────
        ...
        ydl_opts = {
            **base_opts,
            # نجبر mp4 بدون الاعتماد على ffmpeg للدمج قدر الإمكان
            'format': (
        ...
        for cookie_file in ['youtube_cookies.txt', 'cookies.txt']:
            if os.path.exists(cookie_file):
                ydl_opts['cookiefile'] = cookie_file
                break

    # ─── Twitter / Facebook / غيرها ────────────────────────
    else:
        ydl_opts = {
            **base_opts,
    ...
    # ─── إيجاد الملف ───────────────────────────────────────
    # جرب الاسم المتوقع أولاً
    expected = os.path.join(tmp_dir, 'video.mp4')
    if os.path.exists(expected) and os.path.getsize(expected) > 1000:
        filepath = expected
    else:
        filepath = _find_downloaded_file(tmp_dir)

    if not filepath:
        raise FileNotFoundError("لم يُعثر على الملف بعد التحميل")

    # ─── تحويل TikTok إلى mp4 قابل للتشغيل ────────────────
    # TikTok أحياناً ينزّل بامتداد .mp4 لكنه codec غريب
    if platform == 'tiktok' and not filepath.endswith('output_final.mp4'):
        filepath = _convert_to_mp4(filepath, tmp_dir)

    # تحقق من الحجم النهائي
    file_size = os.path.getsize(filepath) / (1024 * 1024)

    return {
        'filepath': filepath,
        'title': info.get('title') or info.get('description', 'فيديو')[:80],
...
def parse_ydl_error(err: str, platform: str = '') -> str:
    """تحويل أخطاء yt-dlp إلى رسائل عربية واضحة."""
    err_lower = err.lower()

    if 'private' in err_lower or 'login' in err_lower or 'requires authentication' in err_lower:
        if platform == 'instagram':
            return (
                "🔒 *الفيديو خاص أو يحتاج تسجيل دخول*\n\n"
                "الانستا يحتاج ملف كوكيز للريلز الخاصة.\n"
                "تواصل مع الأدمن لإضافة الكوكيز.\n\n"
                "✅ الريلز العامة تعمل بدون مشكلة."
            )
        return "🔒 *الفيديو خاص أو يحتاج تسجيل دخول*\n\nما أقدر أحمل هذا الفيديو."

    elif 'age' in err_lower or 'age-restricted' in err_lower:
        return "🔞 *الفيديو مقيد بالعمر*\n\nما أقدر أحمله."

    elif 'not available' in err_lower or 'unavailable' in err_lower:
        return "🚫 *الفيديو غير متاح*\n\nممكن يكون محذوف أو محظور."
        ...
        return "⏳ *طلبات كثيرة جداً*\n\nانتظر دقيقتين وحاول مرة ثانية."

    elif 'http error 403' in err_lower:
        return (
            "🚫 *الوصول مرفوض (403)*\n\n"
            "المنصة رفضت الطلب.\n"
            "جرب بعد قليل أو تواصل مع الأدمن."
        )

    elif 'no video formats' in err_lower or 'no formats' in err_lower or 'requested format not available' in err_lower:
        return (
            "❌ *ما يوجد تنسيق متاح للتحميل*\n\n"
            "ممكن الفيديو صوت فقط أو تنسيقه غير مدعوم."
        )
        ...
        return "⚙️ *خطأ في معالجة الفيديو*\n\nتأكد من تثبيت ffmpeg على السيرفر."

    else:
        return (
            "❌ *فشل التحميل*\n\n"
            "تأكد إن الرابط صحيح وإن الفيديو عام.\n"
            "إذا استمرت المشكلة تواصل مع الأدمن."
        )
